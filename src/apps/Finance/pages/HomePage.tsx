  ...
  return (
    <div className="finance-home-page animate-fade-in" style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', padding: '0 4px' }}>
        <h2 style={{ fontSize: '1.2rem', fontWeight: 700, margin: 0, color: '#f8fafc' }}>{currentMonth}월의 리포트</h2>
        <div style={{ display: 'flex', gap: '16px' }}>
          <Share2 size={22} color="#94a3b8" cursor="pointer" onClick={handleShare} />
          <div style={{ position: 'relative' }}>
            <Bell size={22} color="#94a3b8" cursor="pointer" />
      ...
      <div className="f-card" style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
          <div>
            <div style={{ fontSize: '0.85rem', color: '#94a3b8', marginBottom: '4px' }}>이번 달 결제 예정액</div>
            <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#f8fafc' }}>
              {store.totalExpectedPayment.toLocaleString()}원
            </div>
          </div>
        </div>
        <div style={{ borderTop: '1px solid #1e293b', paddingTop: '16px', display: 'flex', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '2px' }}>월말 예상 잔액</div>
            <div style={{ fontSize: '1rem', fontWeight: 600, color: '#34d399' }}>
              {store.expectedMonthlyBalance.toLocaleString()}원
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '2px' }}>예상 절약 금액</div>
      ...
      {/* Quick Menu */}
      <h3 style={{ fontSize: '1rem', fontWeight: 600, margin: '0 0 16px 4px', color: '#e2e8f0' }}>빠른 메뉴</h3>
        ...
        {[
          ...
          <button 
            ...
            style={{ 
              ...
              background: '#1e293b', border: 'none', borderRadius: '16px', padding: '16px 0',
              cursor: 'pointer', transition: 'background 0.2s'
            }}
          >
            <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#334155', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#f8fafc' }}>
              {menu.icon}
            </div>
            <span style={{ fontSize: '0.8rem', color: '#cbd5e1' }}>{menu.label}</span>
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '0 4px 16px 4px' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: 600, margin: 0, color: '#e2e8f0', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        {store.aiInsights.map((insight: any) => (
          <div key={insight.id} className="f-card" style={{ padding: '16px', borderLeft: insight.isUrgent ? '4px solid #f87171' : '4px solid #3b82f6' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <ChevronRight size={18} color="#475569" style={{ marginTop: '2px' }} />
            </div>
          </div>
        ))}
      </div>
